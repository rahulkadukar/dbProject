<?php
	...
	$sql   = "SELECT * FROM `valve`.`dlc_list` WHERE lastUpdate < ".$time." LIMIT 0,250";
		...
		if($resultStatus['http_code'] == 200){
			...
			if($new[$appid]["success"]){
				...
				if($gameInfo["type"]=="dlc" ){
					...
					$sql = "INSERT INTO `valve`.`dlc_master` (`appID`, `name`, `currency`, `price`, `age`, `website`, `releaseDate`) VALUES ('".$appid."','".mysql_real_escape_string($gameInfo['name'])."','".$gameInfo['price_overview']['currency']."','".$gameInfo['price_overview']['initial']."','".$gameInfo['required_age']."','".$gameInfo['website']."','".$release."')";
					...
					$sql = "UPDATE `valve`.`dlc_list` SET `lastUpdate` =  '".date('U')."' WHERE `DLC` = ".$appid;
					$result = mysql_query($sql);
				}
			}
			else{
				$sql = "INSERT INTO `valve`.`missing_dlc` (`appID` ) VALUES ('".$appid."')";
				$result = mysql_query($sql);
