        ...
        $(document).ready(function(){
			...
			if(uid){
				...
				$.get('steamFetch.php',{'name': uid,},function(userdata){
					...
					$.get('last2weeks.php',{'name': user_array.steamID64},function(data){
						var last_array = JSON.parse(data);
						var minutes2weeks = 0;
						for(i in last_array.response.games)
							minutes2weeks += parseInt(last_array.response.games[i].playtime_2weeks);
						
						minutes2weeks /= 60;
						$('#statsRecent').html(minutes2weeks.toFixed(2) + ' hours played in the last 2 weeks');
					});
				});
		...
		<div id="header">
			<ul>
				<?php
					...
					$length = count($file);
					$new 	= strtoupper($file[$length-2].$file[$length-1]);
					foreach($t_file as $v_file_index => $wa_file)
					{
						$contents = explode(",",$wa_file);
						$file 	= explode("/",$contents[1]);
						$length = count($file);
						$find 	= strtoupper($file[$length-2].$file[$length-1]);
						if(strcmp(trim($new),trim($find))==0)
							echo '<li><a style="color: #41b7d8" href="'.$contents[1].'">'.$contents[0].'</a></li>';
						else
							echo '<li><a href="'.$contents[1].'">'.$contents[0].'</a></li>';
					}
				?>
			</ul>
		</div>
...
<!-- This part is the page content -->	
			<div id="content">
				<div id="title">
				<?php
					$query = 'SELECT quote FROM header_text ORDER BY RAND() LIMIT 0,1';
					$result	= mysql_query($query);
					while($row = mysql_fetch_row($result))
					{				
						echo $row[0];
				...
				<p class="header">Steam Profile Analyser</p>
				
				<div id="dialog" style="display:none">
					<p style="margin:20px">Welcome to the Steam profile analyser. Please wait while your data is being fetched from the mighty Steam servers, 
					this should not take more than 5 seconds under any circumstance. If you are still reading this then you either have a large Steam library 
					or Steam servers are not performing at their peak and it will take more than 5 seconds.</p>
				</div>
				...
				<div id="guide">
					...
					<div id="intro" class="someText">
						...
						<ul>
							...
							<li>If in doubt enter <font style="color: #ccff00;">rahulkadukar</font></li>
						</ul>
						</p>
					</div>
				...
				<div id="stats" class="dataCenters" style="display:none">
			...
			<div id="footer">&copy; Rahul Kadukar 2013</div>
		</div>
	</body>
</html>