...
SELECT a.`location`, b.`appID`, SUM(`playtime`) AS playtime FROM `user_data` AS a JOIN `user_games` AS b ON a.`steamID` = b.`steamID` WHERE `playtime` <> 0 GROUP BY `location`, `appID` ) AS x GROUP BY location