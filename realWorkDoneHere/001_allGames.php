...
/* This file will be used to fetch a list of all games that are currently available on Steam*/
	...
	$size = 500;
	$url  = "http://api.steampowered.com/IPlayerService/GetOwnedGames/v1?steamid=76561198026221141&key=FCCAA3E90D04C71D59EAD2822B2AF90B&include_played_free_games=1";
	$curl = curl_init($url);	
	...
	if($resultStatus['http_code'] == 200){
		...
		foreach($new["response"]["games"] as $game){
			if($incr % $size == 1){
				$sql = " ";
				$sql = "INSERT INTO `valve`.`valve_app_master` (`appID`, `lastUpdate`) VALUES ";
			}
			$sql .= '("'.$game[appid].'","'.$time.'"),';
